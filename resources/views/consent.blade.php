@extends('layouts.app')

@section('content')
    <div class="container text-white bg-dark p-5" style="max-width: 1100px; border-radius: 10px;">


        <h2 class="text-warning text-center">個人情報の取扱い及び転職支援サービス利用に関する同意書</h2>
        <p class="text-center">同意及び合意フォーム</p>

        <p>以下の内容をご確認いただき、同意及び合意の上で確認画面へ進んでください。</p>

        <h4>1. 収集する個人情報について</h4>
        <p>本フォームを通じて、以下の個人情報を収集いたします。</p>
        <ul>
            <li>氏名（フリガナ含む）</li>
            <li>性別</li>
            <li>生年月日および満年齢</li>
            <li>LINE登録名</li>
            <li>住所（郵便番号、都道府県、市区町村、番地）</li>
            <li>住居形態（実家・持家・社宅・社員寮・その他）</li>
            <li>連絡先（メールアドレス、電話番号）</li>
            <li>勤務先情報（会社名、郵便番号、都道府県、住所、連絡先、上司の携帯番号）</li>
            <li>職種および勤続年数</li>
            <li>現在の勤務状況（公休・勤務中・この後勤務・バックレ状態）</li>
            <li>希望退職日、最終出勤日</li>
            <li>有給取得希望</li>
            <li>退職連絡が必要な担当者</li>
            <li>銀行口座情報（銀行名、口座種別、口座番号、口座名義）</li>
            <li>雇用契約書または労働条件通知書（PDF・画像ファイル）</li>
            <li>身分証明書（運転免許証、健康保険証、パスポート等）（PDF・画像ファイル）</li>
        </ul>

        <h4>2. 利用目的</h4>
        <p>取得した個人情報は、以下の目的のために利用します。</p>
        <ul>
            <li>転職支援サービスの提供（求人情報の提供、面接調整、キャリア相談等）</li>
            <li>転職先への情報提供</li>
            <li>現在の勤務先への退職通知のサポート</li>
            <li>人材紹介企業または提携企業への情報提供</li>
            <li>その他、当社が提供する転職支援サービスに付随する業務</li>
            <li>弁護士､社会保険労務士､税理士､公認会計士(これらの士業法人含む)の専門家等への取次</li>
        </ul>

        <h4>3. 個人情報の管理について</h4>
        <p>当社は、お預かりした個人情報の適切な管理を行い、不正アクセス、紛失、漏洩が生じないよう、最大限の安全対策を講じます。</p>

        <h4>4. 合意及び同意の確認</h4>
        <p>私は、上記の内容を理解し、以下の事項に合意及び同意します。</p>
        <ul>
            <li>本フォームに入力した個人情報が、転職支援サービスの提供および勤務先への退職通知のサポートの目的で利用されること</li>
            <li>求人紹介および転職支援の目的で、必要に応じて提携企業や人材紹介会社へ個人情報が提供されること</li>
            <li>当社が定める個人情報の取扱いに関するポリシーに従い、情報が管理されること</li>
            <li>退職代行モーアカン®が貴殿から本サービス受任後､紛争性を有した場合は介入しないこと</li>
            <li>当サービスの判定フォーム又は情報フォームにおいて､貴殿の提供した内容に虚偽や相違が合った場合において基因する損害の責を追わないこと</li>
            <li>残業代及び退職金並びに未払給与の請求は行わないこと</li>
            <li>必ずしも有給消化ができない場合があること</li>
        </ul>

        <form action="{{ route('consent.submit') }}" method="POST">
            @csrf
            <div class="form-check">
                <input type="checkbox" class="form-check-input" id="consent" name="consent" value="1" required>
                <label class="form-check-label" for="consent">
                    <strong>承諾･同意･合意(チェックを入れてください)</strong>
                </label>
            </div>
            <div class="text-center mt-3">
                <button type="submit" class="btn btn-secondary">確認画面へ</button>
            </div>
        </form>

    </div>
@endsection
